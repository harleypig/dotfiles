...
| 5 | Security | **Active (partial)** | Secrets: `gitleaks` + `detect-private-key` (commit-time check) **plus `trufflehog`** — PR-time *verified* scan in CI (`secret-scan.yml`, non-required for now). SCA / supply-chain: Dependabot alerts + version updates (`.github/dependabot.yml`). SAST: `semgrep` via the `security-scan` skill; **Checkmarx evaluated & declined** (commercial, no free tier — disproportionate). `CodeFactor`/`Snyk` run as PR checks — formalizing their use is a separate **Planned** TODO. DAST: **N/A** (no running service). Deeper triage → `security-scan` skill. |
