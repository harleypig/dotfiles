...
| `hashicorp/agent-skills` (official) | First-party agent skills for Terraform/Packer/tflint (style-guide, terraform-test/mock_provider, packer-builders per mining — specific contents unverified). Re-mine on tool version bumps. We authored our own lean rules from the **official docs** instead. | (check) | 2026-06-29 | No (grounded in HashiCorp docs, not this repo) |
| `antonbabenko/terraform-skill` | A polished Terraform agent skill (risk-routing, decision matrices, a "Response Contract" pattern, progressive disclosure). Good structural ideas, but heavier than our rule-first house style; we kept rules + one patterns skill. | (check) | 2026-06-29 | No (ideas only; structure noted, not reused) |
