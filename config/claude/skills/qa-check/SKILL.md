...
- **Whole-codebase dimension reviews** (an across-the-repo pass, not the diff)
  ...
  *runs* them), **a11y-review** (UI/UX & accessibility — UI repos). They
  assess and report; they don't mutate. For Python depth invoke
  **pytest-patterns** / **typing-patterns**. Run these when the change (or the
  request) is about a dimension's health, not on every trivial diff.
- **Stack sub-pipelines** — for a **Terraform** repo, delegate the whole
  ...
  structural audit, belt-and-suspenders over pre-commit) to
  **terraform-review** rather than wiring those steps here; a
  **packer-review** sibling is planned.
