...
| [merge-finalization.py](hooks/merge-finalization.py) | `PreToolUse` on `gh pr merge` / `push.sh merge` | Blocks a merge while completed `- [x]` items remain in the **target repo's** planning docs (opt-in per repo via `merge-finalization: enforce` in `WORKFLOW.md`); resolves `--repo`/`-R` to a sibling clone and honors a `cd <path> &&` prefix — an unresolvable target gets the reminder, never a block |
...
| [secret-echo-guard.py](hooks/secret-echo-guard.py) | `PreToolUse` on `Bash` | Blocks a command that echoes/`printf`s a secret-looking variable's value to the terminal (the "just echo it to check it's set" leak); allows piped / redirected / `$(…)`-captured value-uses and set-marker/length expansions; enforces the global `CLAUDE.md` *Secret Handling* rule |
...
| [STEERING.md](docs/STEERING.md) | The umbrella over every steering mechanism (`CLAUDE.md`, rules, skills, subagents, hooks, output-styles) on three axes — when-it-loads / context cost / authority — and the rule that ties them: pick the cheapest mechanism that carries the needed authority; maps onto this config's three-tier model |
| [SKILLS.md](docs/SKILLS.md) | Skills as folders (instructions + scripts + resources) — the nine skill categories, authoring best-practices and anti-patterns, distribution via marketplaces, and usage measurement; points at `EXTENDING.md` for this repo's own conventions |
| [SUBAGENTS.md](docs/SUBAGENTS.md) | Subagents — isolated context that returns only a summary; when to delegate vs keep in-thread, the built-in types, the five ways to direct one, and per-subagent model+effort choice (see MODELS-AND-EFFORT) |
| [MODELS-AND-EFFORT.md](docs/MODELS-AND-EFFORT.md) | The two orthogonal dials — which model (the capability ladder) vs. how much effort (thoroughness) — the "it's all tokens from one loop" model, the selection heuristic, and a model×effort decision table |
| [PROMPT-CACHING.md](docs/PROMPT-CACHING.md) | Prefix-match prompt caching and the Claude Code patterns that preserve hit-rate — static-before-dynamic ordering, state via messages not system-prompt edits, deferred-tool stubs, no mid-conversation tool/model churn, cache-safe compaction/forking |
| [AGENT-NATIVE-ENGINEERING.md](docs/AGENT-NATIVE-ENGINEERING.md) | Engineering *for* the agent at three altitudes — design tools the way the agent sees them, onboard it like a new developer (context layer + skills + MCP), and restructure the workflow (verification as the new bottleneck); anchored to this repo's own artifacts |
