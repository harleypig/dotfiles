...
# Meta tests for ansi. Basically, tests that don't actually execute the
# script.

load "$BATS_TEST/global.bash"

#############################################################
# Basic Tests for ansi

@test 'ansi exists' {
  assert_file_exist "/nas_pp/etl/dotfiles/bin/ansi"
}

#------------------------------------------------------------
@test 'ansi is in PATH' {
  
  run which ansi
  assert_success
  assert_output "/nas_pp/etl/dotfiles/bin/ansi"
}

#------------------------------------------------------------
@test 'ansi has expected perms (-rwxr-xr-x)' {
  run stat --printf '%A' "/nas_pp/etl/dotfiles/bin/ansi"
  assert_success
  assert_output '-rwxr-xr-x'
}

#------------------------------------------------------------
@test 'ansi has a man page' {
  skip 'documentation not currently required'
  grep -q '^##\s\+NO\s\+MANPAGE$' /nas_pp/etl/dotfiles/bin/ansi && skip 'No man page required'
  run man -w ansi
  assert_success
  assert_output '$GLOBAL_DIR/man/man1/ansi.1'
}
...
@test 'ansi shellcheck passes' {
  ...
  run shellcheck "${switches[@]}" "/nas_pp/etl/dotfiles/bin/ansi" &> /dev/null
  assert_success
  assert_output ''
}
...
@test 'ansi shfmt passes' {
  ...
  # We are only interested in success/fail here.
  run shfmt "${switches[@]}" "/nas_pp/etl/dotfiles/bin/ansi" &> /dev/null
  assert_success
  assert_output ''
}